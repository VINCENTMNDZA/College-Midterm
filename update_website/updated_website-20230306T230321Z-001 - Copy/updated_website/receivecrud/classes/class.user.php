...
class User{
	...
	function get_prod_image($id){
		$sql="SELECT prod_image FROM tbl_users WHERE user_id = :id";	
		$q = $this->conn->prepare($sql);
		$q->execute(['id' => $id]);
	...
	public function update_product_img($fname,$id){
		...
		$sql = "UPDATE tbl_users SET prod_image=:prod_image,user_date_updated=:user_date_updated,user_time_updated=:user_time_updated WHERE user_id=user_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':prod_image'=>$fname, ':type_date_updated'=>$NOW,':type_time_updated'=>$NOW,':user_id'=>$id));
		return true;
}
}